...
class Pool_table_Dataset(Dataset):
    ...
    Each of those directories should contain image files. This class applies basic
    transformations such as resizing and normalization.
    """

    def __init__(self, metadata:list[dict], transform: Optional[transforms.Compose] = None):
...
def build_regression_model(backbone='resnet18', pretrained=True):
    if backbone == 'resnet18':
        ...
        model.fc = nn.Sequential(
            nn.Dropout(0.3),
            nn.Linear(model.fc.in_features, 1)   # single count output
        )
...
def parse_args():
    ...
    parser.add_argument("--backbone", type=str, required=True, help="Architecture of the cnn model")
...
if __name__ == "__main__":
    ...
    if args.train:
        ...
        for epoch in range(args.epochs):
            ...
            # if device.type == 'cpu': # this is usefull for limited memory scenarios
            #     gc.collect()
        #--------------- TRAINING BLOCK ---------------
    ...
    mae, mape, mse = evaluate_model(model, test_loader, device)
